...
| 實踐軸（Practice Axis） | FinMind 微產出 + 交付物 checklist | 自評勾選（deliverableDone）| ⚠️ G 科目已有引導；驗證仍為手動 |
...
**目前實踐軸現況**：
- G1–G16 各模組均已定義 4 項明確的交付物 checklist（`finmath_advanced_checks` key），學習者可勾選自評並透過 ProgressKey 跨裝置保存進度
- A–F 18 章的 FinMind 程式碼已全面對齊 API 語法並加入 Python 語法高亮
- L3（自動輸出驗證）仍完全空白，目前僅靠學習者自評，系統無法主動判斷輸出是否符合機構標準
...
| 層次 | 描述 | 目前狀態 |
|------|------|----------|
| L1. 程式碼可執行 | 每章的 `finmindCode` 能在本機無錯執行，輸出有意義的結果 | ✅ A–F 全 18 章 API 語法已對齊；Python 語法高亮已加 |
| L2. 任務引導 | 每章有明確的「你要計算什麼、用哪個 API、輸出什麼格式」的任務說明 | ✅ G1–G16 各有 4 項交付物 checklist；A–F 有 micro-deliverable 文字描述 |
| L3. 輸出驗證 | 平台能判斷學習者的輸出是否合格（自評清單或自動比對） | ❌ 完全空白，仍為純手動自評勾選 |
