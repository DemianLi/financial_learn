...
const syllabusData = {
  ...
  "topics": [
    ...
    },
    {
      ...
      "examQuestions": [
        {
          "question": "假設有三家晶圓代工同業的本益比（P/E）與預期營收成長率組成的二維向量分別為：公司甲 $\\mathbf{u}_1 = [25, 20\\%]^T$，公司乙 $\\mathbf{u}_2 = [18, 12\\%]^T$，公司丙 $\\mathbf{u}_3 = [15, 8\\%]^T$。這三點在二維平面上呈現極佳的線性關係，建構出一條「成長估值投影線」 $P/E = 100 \\times \\text{成長率} + 6$。若目標公司丁的預期營收成長率為 15%，根據同業多維特徵投影線，公司丁的合理 P/E 應為多少倍？",
          "options": [
            "合理 P/E = 15 倍",
            "合理 P/E = 21 倍",
            "合理 P/E = 18 倍",
    ...
    },
    {
      ...
      "finmindCode": "import numpy as np\nimport pandas as pd\nfrom FinMind.data import DataLoader\n\ndl = DataLoader()\n\n# ── 1. 真實 Beta 計算（verified column: close）──\n# 用元大台灣50 ETF(0050) 作市場替代，避開無命名方法的 TAIEX API\ndf_stock = dl.taiwan_stock_daily(stock_id='2330', start_date='2023-01-01', end_date='2024-12-31')\ndf_mkt = dl.taiwan_stock_daily(stock_id='0050', start_date='2023-01-01', end_date='2024-12-31')\n\ndf_ret = (\n    df_stock.set_index('date')[['close']].rename(columns={'close': 'stock'})\n    .join(df_mkt.set_index('date')[['close']].rename(columns={'close': 'market'}))\n    .pct_change().dropna()\n)\nbeta = df_ret['stock'].cov(df_ret['market']) / df_ret['market'].var()\nprint(f\"實際 Beta（2330 vs 0050 ETF，2年）: {beta:.2f}\")\n\n# ── 2. WACC（Rf 來自台灣 10 年期公債，FinMind GovernmentBondsYield 欄位未公開，請手動查詢 TWSE）──\nRf = 0.018       # 台灣 10Y 公債殖利率（2024 年底約 1.8%，需定期更新）\nRm = 0.08        # 台股市場年化報酬率（歷史平均）\nRd = 0.025       # 借款成本\ntax_rate = 0.20  # 企業所得稅率\nD_E = 0.3        # 負債/股東權益比（由財報估算）\n\nKe = Rf + beta * (Rm - Rf)  # CAPM 股東要求報酬率\nKd = Rd * (1 - tax_rate)    # 稅後債務成本\nwe = 1 / (1 + D_E)\nwd = D_E / (1 + D_E)\nWACC = we * Ke + wd * Kd\nprint(f\"Ke={Ke:.2%}, Kd={Kd:.2%} → WACC={WACC:.2%}\")\n\n# ── 3. DCF 估值 + WACC±1% 敏感度（FCF 為示範值，可替換為 A3 章節取得的真實 FCF）──\nfcf = np.array([100, 115, 132, 152, 175])  # 億元\ng = 0.025\n\ndef dcf(fcf, wacc, g):\n    n = len(fcf)\n    pv_fcf = sum(fcf[i] / (1 + wacc) ** (i + 1) for i in range(n))\n    pv_tv = fcf[-1] * (1 + g) / (wacc - g) / (1 + wacc) ** n\n    return pv_fcf + pv_tv\n\nprint(f\"\\nDCF 敏感度分析:\")\nfor w in [WACC - 0.01, WACC, WACC + 0.01]:\n    label = '基準' if abs(w - WACC) < 1e-9 else ('+1%' if w > WACC else '-1%')\n    print(f\"  WACC {w:.2%}（{label}）: EV = {dcf(fcf, w, g):.0f} 億元\")",
      "examQuestions": [
        {
    ...
    },
    {
      ...
      "finmindCode": "import pandas as pd\nfrom FinMind.data import DataLoader\n\ndl = DataLoader()\n\n# Part 1: 成長股 — PE + YoY成長率 二維坐標 & PEG（核心考點）\ngrowth_stocks = ['2330', '2454']  # 台積電, 聯發科\nresults = []\n\nfor stock in growth_stocks:\n    # 月營收 YoY 成長率（verified 欄位: revenue, revenue_year）\n    df_rev = dl.taiwan_stock_month_revenue(\n        stock_id=stock, start_date='2023-01-01', end_date='2024-12-31'\n    )\n    yoy = None\n    if not df_rev.empty:\n        annual = df_rev.groupby('revenue_year')['revenue'].sum()\n        if len(annual) >= 2:\n            yoy = (annual.iloc[-1] / annual.iloc[-2] - 1) * 100\n\n    # 最新 PE（verified 欄位: PER）\n    df_pe = dl.taiwan_stock_per_pbr(stock_id=stock, start_date='2024-12-01')\n    per = None\n    if not df_pe.empty:\n        per = df_pe.sort_values('date').iloc[-1]['PER']\n\n    if yoy and per and yoy > 0:\n        peg = per / yoy  # PEG = P/E ÷ 成長率%，<1 表示相對低估\n        quadrant = ('高成長-高估值' if per > 20 and yoy > 15 else\n                    '高成長-低估值' if yoy > 15 else\n                    '低成長-高估值(成長價值失衡)' if per > 20 else '低成長-低估值')\n        valuation = '偏便宜' if peg < 1 else ('合理' if peg <= 1.5 else '偏貴')\n        results.append({'stock_id': stock, 'YoY成長%': round(yoy, 1),\n                        'P/E': round(per, 1), 'PEG': round(peg, 2),\n                        '象限': quadrant, '估值': valuation})\n\nprint(\"成長股 PE-成長率 二維坐標 & PEG:\")\nprint(pd.DataFrame(results).to_string(index=False))\n\n# Part 2: 景氣循環股 — PE 坐標發散示範（估值基底必須切換為 P/B）\ndf_steel = dl.taiwan_stock_per_pbr(stock_id='2002', start_date='2024-01-01')\ndf_steel['估值基底'] = df_steel['PER'].apply(\n    lambda x: 'PE可用' if 0 < x < 50 else 'PE發散→需切換P/B'\n)\nprint(\"\\n中鋼(2002) 景氣循環股 — 估值基底切換示範:\")\nprint(df_steel[['date', 'PER', 'PBR', '估值基底']].tail(10))",
      "examQuestions": [
        {
    ...
    },
    {
      ...
      "finmindCode": "import pandas as pd\nfrom FinMind.data import DataLoader\n\ndl = DataLoader()\n\n# 獲取台股個股融資融券數據\ndf_margin = dl.taiwan_stock_margin_purchase_short_sale(\n    stock_id='2330',\n    start_date='2025-01-01',\n    end_date='2025-12-31'\n)\n\n# MarginPurchaseToday = 當日融資餘額, ShortSaleToday = 當日融券餘額\ndf_margin['券資比'] = df_margin['ShortSaleTodayBalance'] / df_margin['MarginPurchaseTodayBalance']\n\nprint(\"台積電信用交易與券資比變動力學:\")\nprint(df_margin[['date', 'MarginPurchaseTodayBalance', 'ShortSaleTodayBalance', '券資比']].tail())",
      "examQuestions": [
        {
    ...
    },
    {
      ...
      "finmindCode": "import pandas as pd\nfrom FinMind.data import DataLoader\n\ndl = DataLoader()\n\n# 獲取集保戶股權分散表（股權持股分級表）\ndf_share = dl.taiwan_stock_holding_shares_per(\n    stock_id='2330',\n    start_date='2025-10-01'\n)\n\nlatest_date = df_share['date'].max()\ndf_latest = df_share[df_share['date'] == latest_date]\n\n# HoldingSharesLevel = 持股等級, people = 持股人數, unit = 持股張數\nmax_stage = df_latest['HoldingSharesLevel'].max()\nclass_1000 = df_latest[df_latest['HoldingSharesLevel'] == max_stage]\n\nprint(f\"最新集保公告日期: {latest_date}\")\nprint(\"千張大戶持股結構:\")\nprint(class_1000[['HoldingSharesLevel', 'people', 'unit', 'percent']])",
      "examQuestions": [
        {
    ...
    },
    {
      ...
      "finmindCode": "import pandas as pd\nfrom FinMind.data import DataLoader\n\ndl = DataLoader()\n\n# Step 1: 取得真實新聞（verified columns: date, title, description, source, link）\ndf_news = dl.taiwan_stock_news(\n    stock_id='2330',\n    start_date='2025-12-01'\n)\n\n# Step 2: 取近 5 則，提供閱讀材料\ndf_sample = df_news.sort_values('date', ascending=False).head(5).reset_index(drop=True)\nprint(\"══ 台積電近期新聞（請閱讀後手動標注情緒）══\")\nfor i, row in df_sample.iterrows():\n    print(f\"[{i+1}] {row['date']} | {row['source']}\")\n    print(f\"    標題: {row['title']}\")\n    print(f\"    摘要: {str(row['description'])[:80]}...\")\n    print()\n\n# Step 3: 空白標注模板 — 學習者在 sentiment 欄填入 +1(正面) / 0(中性) / -1(負面)\nlabeling_template = pd.DataFrame({\n    'index': range(1, len(df_sample) + 1),\n    'title': df_sample['title'].values,\n    'sentiment': ['__' for _ in range(len(df_sample))],  # 學習者填入\n    'reason': ['__' for _ in range(len(df_sample))]       # 一句理由\n})\nprint(\"══ 手動標注模板（在 sentiment 欄填入 +1/0/-1）══\")\nprint(labeling_template.to_string(index=False))\nprint(\"\\n完成標注後，情緒向量即為 BERT 的人工標注基準（Ground Truth）。\")",
      "examQuestions": [
        {
    ...
    },
    {
      ...
      "finmindCode": "import pandas as pd\nfrom FinMind.data import DataLoader\n\ndl = DataLoader()\n\n# Step 1: 歷史月營收 YoY（verified column: revenue）\ndf_rev = dl.taiwan_stock_month_revenue(\n    stock_id='2330', start_date='2022-01-01'\n)\ndf_rev = df_rev.sort_values('date').reset_index(drop=True)\ndf_rev['YoY'] = df_rev['revenue'].pct_change(12)\ndf_rev = df_rev.dropna(subset=['YoY'])\n\n# Step 2: 先驗 P(H) — 歷史上 YoY > 15% 的月份比例\nTH = 0.15\ndf_rev['is_beat'] = df_rev['YoY'] > TH\nP_H = df_rev['is_beat'].mean()\nprint(f\"先驗 P(Beat) = {P_H:.1%}（歷史 YoY > {TH:.0%} 月份佔比）\")\n\n# Step 3: 似然度 — 以「YoY 高於歷史均值」作為正面信號 E\nmean_yoy = df_rev['YoY'].mean()\ndf_rev['is_signal'] = df_rev['YoY'] > mean_yoy\nP_E_H = df_rev[df_rev['is_beat']]['is_signal'].mean()\nP_E_nH = df_rev[~df_rev['is_beat']]['is_signal'].mean()\nprint(f\"P(正面信號 | Beat)     = {P_E_H:.1%}\")\nprint(f\"P(正面信號 | Not Beat) = {P_E_nH:.1%}\")\n\n# Step 4: 觀測最新月信號\nlatest = df_rev.iloc[-1]\nE = bool(latest['is_signal'])\nprint(f\"\\n最新月 YoY={latest['YoY']:.1%}，歷史均值={mean_yoy:.1%}，信號={'正面' if E else '負面'}\")\n\n# Step 5: 貝氏更新 P(H|E)\nP_E = P_E_H * P_H + P_E_nH * (1 - P_H)\nif E:\n    P_H_E = P_E_H * P_H / P_E\nelse:\n    P_H_E = (1 - P_E_H) * P_H / (1 - P_E)\n\nprint(f\"\\n── 貝氏更新結果 ──\")\nprint(f\"先驗  P(Beat)          = {P_H:.1%}\")\nprint(f\"後驗  P(Beat | Signal) = {P_H_E:.1%}\")\nprint(f\"更新方向: {'↑ 上修' if P_H_E > P_H else '↓ 下修'} {abs(P_H_E - P_H):.1%}\")",
      "examQuestions": [
        {
    ...
    },
    {
      ...
      "finmindCode": "import pandas as pd\nfrom FinMind.data import DataLoader\n\ndl = DataLoader()\nstock_id = '2317'  # 鴻海\n\n# Part 1: 集保大戶持股比例（verified columns: percent, unit, people）\ndf_share = dl.taiwan_stock_holding_shares_per(\n    stock_id=stock_id, start_date='2025-10-01'\n)\nlatest_date = df_share['date'].max()\ndf_latest = df_share[df_share['date'] == latest_date].copy()\n\n# 以人均持股（unit/people）排序識別大戶等級，避免依賴未驗證的 HoldingSharesLevel 字串值\ndf_latest['avg_unit'] = df_latest['unit'] / df_latest['people'].clip(lower=1)\ndf_sorted = df_latest.sort_values('avg_unit', ascending=False)\ntop_holder_pct = df_sorted.head(3)['percent'].sum()\n\n# Part 2: 融資融券券資比（verified: MarginPurchaseTodayBalance, ShortSaleTodayBalance）\ndf_margin = dl.taiwan_stock_margin_purchase_short_sale(\n    stock_id=stock_id, start_date='2025-01-01', end_date='2025-12-31'\n)\ndf_margin['券資比'] = (df_margin['ShortSaleTodayBalance'] /\n                       df_margin['MarginPurchaseTodayBalance'].clip(lower=1) * 100)\nlatest_margin = df_margin.sort_values('date').iloc[-1]\n\n# Part 3: 當前股價 & 斷頭邊界（verified column: close）\ndf_price = dl.taiwan_stock_daily(stock_id=stock_id, start_date='2025-12-01')\ncur_price = df_price.sort_values('date').iloc[-1]['close']\nmargin_call_price = round(cur_price * 0.78, 1)  # 融資成數60% × 維持率130% = 78%\n\nprint(f\"集保大戶持股（前三大等級合計）: {top_holder_pct:.1f}%\")\nprint(f\"最新券資比: {latest_margin['券資比']:.2f}%\")\nprint(f\"當前股價: {cur_price} 元，斷頭邊界估算: {margin_call_price} 元\")\n\n# 聯合籌碼健康度判斷\nhealth = '健康' if top_holder_pct > 65 and latest_margin['券資比'] < 10 else '存在隱憂'\nprint(f\"\\n籌碼結構: {health}（大戶持股穩定 + 券資比低 = 多頭籌碼安全）\")",
      "examQuestions": [
        {
...
// 每章微產出任務（從 app.js 移至此處，緊鄰章節定義）
// L1=可執行, L2=任務引導, L3=輸出驗證 三層架構
