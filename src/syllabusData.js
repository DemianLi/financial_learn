...
const syllabusData = {
  ...
  "topics": [
    ...
    },
    {
      ...
      "finmindCode": "import pandas as pd\nfrom FinMind.data import DataLoader\n\ndl = DataLoader()\n\n# 獲取台股個股融資融券數據\ndf_margin = dl.taiwan_stock_margin_purchase_short_sale(\n    stock_id='2330',\n    start_date='2025-01-01',\n    end_date='2025-12-31'\n)\n\n# MarginPurchaseToday = 當日融資餘額, ShortSaleToday = 當日融券餘額\ndf_margin['券資比'] = df_margin['ShortSaleToday'] / df_margin['MarginPurchaseToday']\n\nprint(\"台積電信用交易與券資比變動力學:\")\nprint(df_margin[['date', 'MarginPurchaseToday', 'ShortSaleToday', '券資比']].tail())",
      "examQuestions": [
        {
    ...
    },
    {
      ...
      "finmindCode": "import pandas as pd\nfrom FinMind.data import DataLoader\n\n# 1. 初始化 FinMind API 資料載入器\ndl = DataLoader()\n\n# 2. 獲取台股鴻海(2317)的融資融券信用交易數據\ndf_margin = dl.taiwan_stock_margin_purchase_short_sale(\n    stock_id='2317',\n    start_date='2025-01-01',\n    end_date='2025-12-31'\n)\n\n# 3. 計算信用交易槓桿強度：融券餘額與融資餘額比值 (資券比)\n# MarginPurchaseToday = 當日融資餘額, ShortSaleToday = 當日融券餘額\ndf_margin['Margin_Short_Ratio'] = df_margin['ShortSaleToday'] / df_margin['MarginPurchaseToday']\n\n# 4. 計算散戶槓桿重力指標：融資買進餘額佔比\ndf_margin['Leverage_Intensity'] = df_margin['MarginPurchaseToday'] / df_margin['MarginPurchaseLimit']\n\nprint(\"鴻海信用槓桿與反饋重力特徵指標:\")\nprint(df_margin[['date', 'MarginPurchaseToday', 'Margin_Short_Ratio', 'Leverage_Intensity']].tail(5))",
      "examQuestions": [
        {
