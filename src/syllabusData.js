...
const syllabusData = {
  ...
  "topics": [
    ...
    },
    {
      ...
      "finmindCode": "import pandas as pd\nfrom FinMind.data import DataLoader\n\ndl = DataLoader()\n# 可選：dl.login(user_id='YOUR_ID', password='YOUR_PASSWORD')\n\n# 獲取台股多隻同業股票的日交易指標\npeers = ['2330', '2303', '5347']\npeer_data = []\n\nfor stock in peers:\n    df_pe = dl.taiwan_stock_per_pbr(stock_id=stock, start_date='2025-12-01')\n    if not df_pe.empty:\n        latest = df_pe.sort_values(by='date').iloc[-1]\n        peer_data.append({\n            'stock_id': stock,\n            'PE': latest['PER'],\n            'PB': latest['PBR'],\n            'Yield': latest['dividend_yield']\n        })\n\ndf_peers = pd.DataFrame(peer_data)\nmean_pe = df_peers['PE'].mean()\nmean_pb = df_peers['PB'].mean()\n\nprint(\"同業多維投影矩陣:\")\nprint(df_peers)\nprint(f\"\\n投影基準 P/E: {mean_pe:.2f}x, P/B: {mean_pb:.2f}x\")",      "examQuestions": [
        {
    ...
    },
    {
      ...
      "finmindCode": "import pandas as pd\nfrom FinMind.data import DataLoader\n\ndl = DataLoader()\n# 可選：dl.login(user_id='YOUR_ID', password='YOUR_PASSWORD')\n\n# 獲取重資產景氣循環股歷史數據\ndf_steel = dl.taiwan_stock_per_pbr(stock_id='2002', start_date='2024-01-01')\n\ndf_steel['PE_is_stable'] = df_steel['PER'].apply(lambda x: \"發散\" if x > 50 or x < 0 else \"穩定\")\ndf_steel['PB_is_stable'] = df_steel['PBR'].apply(lambda x: \"穩定\" if 0.5 < x < 3.0 else \"異常\")\n\nprint(\"中鋼(2002) 估值基底穩定度抽樣:\")\nprint(df_steel[['date', 'PER', 'PE_is_stable', 'PBR', 'PB_is_stable']].tail(10))",      "examQuestions": [
        {
    ...
    },
    {
      ...
      "finmindCode": "import pandas as pd\nfrom FinMind.data import DataLoader\n\n# 1. 初始化 FinMind API 資料載入器\ndl = DataLoader()\n\n# 2. 獲取台股鴻海(2317)的融資融券信用交易數據\ndf_margin = dl.taiwan_stock_margin_purchase_short_sale(\n    stock_id='2317',\n    start_date='2025-01-01',\n    end_date='2025-12-31'\n)\n\n# 3. 計算信用交易槓桿強度：融券餘額與融資餘額比值 (資券比)\n# MarginPurchaseToday = 當日融資餘額, ShortSaleToday = 當日融券餘額\ndf_margin['Margin_Short_Ratio'] = df_margin['ShortSaleTodayBalance'] / df_margin['MarginPurchaseTodayBalance']\n\n# 4. 計算散戶槓桿重力指標：融資買進餘額佔比\ndf_margin['Leverage_Intensity'] = df_margin['MarginPurchaseTodayBalance'] / df_margin['MarginPurchaseLimit']\n\nprint(\"鴻海信用槓桿與反饋重力特徵指標:\")\nprint(df_margin[['date', 'MarginPurchaseTodayBalance', 'Margin_Short_Ratio', 'Leverage_Intensity']].tail(5))",      "examQuestions": [
        {
